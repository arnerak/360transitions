...
public:
	...
	double getAvgTileQuality() const
	{
		double avg = 0;
		for (auto it = tileQuality.begin(); it != tileQuality.end(); it++)
			avg += it->second;
		return avg / tileQuality.size();
	}
	...
	auto download(int tile, int segment = -1)
	{
		...
		std::shared_ptr<httplib::Response> res = NULL;
		long long duration;
		while (!res)
		{
			auto timer = TIME_NOW_EPOCH_MS;
			res = httpClient->Get(mpd->getUrl(currentSegment, tile, tileQuality[tile]).c_str());
