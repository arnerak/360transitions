...
void downloadNaive()
{
	...
	for (int s = 0; s < numSegments; s++)
	{
		au->downloadTiles(s,highQuality);
	}
}

void downloadTrace(std::string pathToTrace)
{
...
int main(int argc, char* argv[])
{
	...
	const int numEvalTraces = 30;
	const char* rps[1] = { "LFUDA" };
	...
	csv << "Cache Size (MB),Type,Stable State,BHR,CHR\n";

	tracesUsed << "== EVAL ==" << std::endl;
	auto testTraces = tracePermutation(numEvalTraces);
	for (auto traceStr : testTraces)
		tracesUsed << strrchr(traceStr.c_str(), '/') << " ";
	tracesUsed << "\n";

	tracesUsed << "== CACHE INIT ==" << std::endl;
	...
	for (int i = 0; i < numStableStates; i++)
	{
		...
		for (int c = 0; c < 4; c++)
		{
			...
			std::cout << "Stable State: " << i << "; " << cacheSize << std::endl;
			editSquidConf("LFUDA", cacheSize);
			resetSquidCache();
			initCache(traces);
			au->resetCacheHitrateVars();
			downloadPopularTiles();
			csv << cacheSize << ",Popular," << i << "," << au->byteHitrate() << "," << au->cacheHitrate() << "\n";
			int j = 0;
			for (auto trace : testTraces)
			{
				au->resetCacheHitrateVars();
				downloadTrace(trace);
				csv << cacheSize << ",Prediction," << i << "," << au->byteHitrate() << "," << au->cacheHitrate() << "\n";
				std::cout << "\r" << ++j << "/" << numEvalTraces << std::flush;
			}
			std::cout << std::endl;
			au->resetCacheHitrateVars();
			downloadNaive();
			csv << cacheSize << ",Naive," << i << "," << au->byteHitrate() << "," << au->cacheHitrate() << "\n";
		}
	}
	csv.close();
	tracesUsed.close();
